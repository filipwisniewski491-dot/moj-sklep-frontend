...
function CategoryContent() {
  ...
  const slugArray = Array.isArray(routerParams.slug) ? routerParams.slug : [routerParams.slug].filter(Boolean);
  const fullPath = slugArray.join('/');
  
  const [categoryData, setCategoryData] = useState<any>(null);
  const [products, setProducts] = useState<any[]>([]);
  
  // Niezależny stan dla filtrów
  ...
  useEffect(() => { setDisplayLimit(24); }, [searchParams]);

  // Pobieranie struktury kategorii i samych produktów (Szybko)
  useEffect(() => {
    async function fetchCategoryData() {
      ...
      try {
        ...
        const rawFaqs = json.category?.faqs || json.faqs || [];
        if (Array.isArray(rawFaqs)) setFaqs(rawFaqs);
        else if (typeof rawFaqs === 'string') { try { setFaqs(JSON.parse(rawFaqs)); } catch (e) {} }
  ...
  // Niezależne, asynchroniczne pobieranie pełnego drzewa filtrów z nowego endpointu
  useEffect(() => {
            ...
            {loading ? (
              ...
              <div className={isListView ? "space-y-4 w-full" : "grid grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4 lg:gap-8"}>
                {products.map((product: any, idx: number) => (
                  <ProductCard key={product.id || product.sku} product={product} isListView={isListView} idx={idx} />
                ))}
              </div>
            )}
          </div>
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 gap-4">
        ...
        <p className="text-xs font-black uppercase tracking-widest text-slate-400">Ładowanie kategorii...</p>
      </div>
    }>
      <CategoryContent />
    </Suspense>
