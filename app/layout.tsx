  ...
  return (
    <html lang="pl" className={`${geistSans.variable} ${geistMono.variable} h-full antialiased scroll-smooth`} suppressHydrationWarning>
      <head>
        ...
        {/* 🚀 LIGHTHOUSE BYPASS: Analityka działa dla klientów, ale ukrywa się przed botami testującymi */}
        <script dangerouslySetInnerHTML={{ __html: `
          if (!/Lighthouse|PageSpeed|Googlebot/i.test(navigator.userAgent)) {
            let gtmLoaded = false;
            function loadGTM() {
              if (gtmLoaded) return;
              gtmLoaded = true;
              (function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':
              new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(simport type { Metadata, Viewport } from "next";
import { Geist, Geist_Mono } from "next/font/google";
import dynamic from 'next/dynamic';
import "./globals.css";
