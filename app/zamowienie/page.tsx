...
export default function CheckoutPage() {
  ...
  const deliveryCost = totalBrutto > 500 ? 0 : 25;
  const finalAmount = totalBrutto + deliveryCost;
        ...
        <form onSubmit={handleFinishOrder} className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <section className="bg-white rounded-3xl p-6 lg:p-8 shadow-sm border border-slate-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input required type="email" placeholder="E-mail (do wysłania faktury)" className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 outline-none focus:border-red-600 transition-colors text-sm font-medium" />
                <input required type="tel" placeholder="Numer telefonu (dla kuriera)" className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 outline-none focus:border-red-600 transition-colors text-sm font-medium" />
              </div>
              ...
              <div className="mt-6 space-y-4">
                {orderType === 'company' && (
                  <div className="relative flex items-center">
                    <input required type="text" placeholder="NIP (Pobierzemy dane z bazy GUS)" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-4 outline-none focus:border-red-600 transition-colors text-sm font-black tracking-widest pr-36" />
                    {/* 🔥 FIX 1: Mocny, kontrastowy przycisk pobierania z GUS */}
                ...
                <input required type="text" placeholder={orderType === 'company' ? "Pełna nazwa firmy / gospodarstwa" : "Imię i nazwisko"} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 outline-none focus:border-red-600 transition-colors text-sm font-medium" />
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <input required type="text" placeholder="Ulica i numer" className="md:col-span-2 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 outline-none focus:border-red-600 transition-colors text-sm font-medium" />
                  <input required type="text" placeholder="Kod pocztowy" className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 outline-none focus:border-red-600 transition-colors text-sm font-medium" />
                </div>
                <input required type="text" placeholder="Miejscowość" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 outline-none focus:border-red-600 transition-colors text-sm font-medium" />
                
                {/* 🔥 FIX 2: Pole na uwagi dla kuriera (Niezbędne w Agro) */}
                <textarea 
          ...
          <aside className="lg:col-span-4 lg:sticky lg:top-24 space-y-6">
            <div className="bg-white rounded-3xl p-6 shadow-xl border border-slate-100">
              ...
              <div className="space-y-2.5 text-xs font-bold text-slate-500 border-t pt-4">
                <div className="flex justify-between"><span>Produkty (netto):</span><span className="text-slate-800">{(totalBrutto / 1.23).toFixed(2)} zł</span></div>
                <div className="flex justify-between"><span>Podatek VAT (23%):</span><span className="text-slate-800">{(totalBrutto - (totalBrutto / 1.23)).toFixed(2)} zł</span></div>
              ...
              >
                {isProcessing ? (
                  <span className="flex items-center gap-2"><div className="w-4 h-4 border-2 border-slate-400 border-t-transparent rounded-full animate-spin"></div> Generowanie...</span>
                ) : (
                  <><span>POTWIERDZAM I PŁACĘ</span><span className="text-xl">➔</span></>
                )}
              </button>

              {/* 🔥 FIX 3: Czytelny i zgodny z prawem tekst prawny z linkami */}
