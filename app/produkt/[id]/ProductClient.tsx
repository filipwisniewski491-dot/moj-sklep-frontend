        ...
        <div className="bg-white rounded-[32px] p-6 lg:p-12 shadow-sm border border-slate-100 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          ...
          <div className="flex flex-col h-full justify-start">
            ...
            <div className="mb-6 flex flex-col md:flex-row md:items-start md:justify-between gap-6">
               <div>
                 {/* Zaktualizowana, prestiżowa etykieta VIP */}
                 {currentTier.level > 1 && (
                   <div className="flex items-center gap-3 mb-2">
                     <div className="inline-flex items-center gap-1.5 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 border border-amber-500/40 px-3 py-1.5 rounded-xl shadow-lg relative overflow-hidden group cursor-default">
                        {/* Interaktywny błysk przy najechaniu */}
                        <div className="absolute inset-0 w-[200%] h-full bg-gradient-to-r from-transparent via-white/10 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000 skew-x-12"></div>
                        <span className="text-amber-400 text-sm">👑</span>
                        <span className="text-[10px] font-black uppercase text-amber-400 tracking-widest">{currentTier.name}</span>
                        <span className="bg-gradient-to-r from-amber-400 to-yellow-500 text-slate-900 px-2 py-0.5 rounded-md text-[10px] font-black shadow-inner ml-1 border border-yellow-200/50">
                          -{currentTier.discountPercent * 100}%
                        </span>
                     </div>
                     {/* Przekreślona stara cena (lepiej widoczna) */}
                     <span className="text-sm text-slate-400 line-through font-black decoration-red-500/60 decoration-2">{numPrice.toFixed(2)} zł</span>
                   </div>
                 )}
               ...
               <div className="flex-1 md:max-w-[280px] flex flex-col gap-2">
                 ...
                 <div className="text-center bg-emerald-50 border border-emerald-100 rounded-xl py-2 px-3 mt-1">
