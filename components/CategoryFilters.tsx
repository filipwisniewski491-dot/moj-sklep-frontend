...
export default function CategoryFilters({ baseFilters = {}, narrowedFilters = {}, totalCount = 0 }: any) {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('shallow-routing', handleUrlChange);
  ...
  useEffect(() => {
    if (isGarageActive && garageBrand) {
      if (localParams.get('Pasuje do marki') !== garageBrand || localParams.get('Pasuje do modelu') !== garageModel) {
        const currentParams = new URLSearchParams(localParams.toString());
        currentParams.set('Pasuje do marki', garageBrand);
        if (garageModel) currentParams.set('Pasuje do modelu', garageModel);
        pushShallow(currentParams);
      }
    }
  }, [isGarageActive, garageBrand, garageModel]);
  ...
  const toggleUrlParam = (key: string, value: string) => {
    ...
    if (valuesArray.length > 0) currentParams.set(key, valuesArray.join(','));
  ...
  const isMarkaSelected = !!localParams.get('Pasuje do marki');
  ...
  localParams.forEach((val, key) => { if (!['limit', 'sort', 'view', 'Pasuje do marki', 'Pasuje do modelu', 'q', 'minPrice', 'maxPrice'].includes(key)) activeFiltersCount++; });
  ...
  const renderFilterBlock = (filterKey: string) => {
    ...
    const hasActiveSelection = !!localParams.get(filterKey);
