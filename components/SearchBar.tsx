...
export default function SearchBar() {
  ...
  // 🚀 Zaktualizowany useEffect - łączy się z naszym tunelem i czyta "hits"
  useEffect(() => {
    if (query.length < 2) {
      setResults([]);
      setIsOpen(false);
    ...
    const delayDebounceFn = setTimeout(async () => {
      ...
      try {
        ...
        // Zmieniono na json.hits (struktura Meilisearch)
              ...
              <div className="flex flex-col">
                ...
                <ul className="max-h-[60vh] overflow-y-auto custom-scrollbar">
                  {results.map((product: any) => {
                    // Odczyt obrazka ze struktury Meilisearch
                    ...
                    return (
                      <li key={product.id || product.sku} className="border-b border-slate-50 last:border-0 hover:bg-red-50/30 transition-colors">
                        {/* Zmieniono na product.handle zgodnie z wynikami Meilisearch */}
                        <Link 
                        ...
                        >
                          <div className="w-14 h-14 bg-white rounded-xl border border-slate-100 flex-shrink-0 flex items-center justify-center overflow-hidden p-1">
                            {img ? <img src={img} alt={product.title || product.name} className="w-full h-full object-contain mix-blend-multiply" /> : <span className="text-[8px] font-black uppercase text-slate-300">Brak</span>}
                          </div>
                          <div className="flex-1 min-w-0">
                            {/* Zmieniono na product.title zgodnie z wynikami Meilisearch */}
